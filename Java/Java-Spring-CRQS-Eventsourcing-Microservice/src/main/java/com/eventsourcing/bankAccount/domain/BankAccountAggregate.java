...
import com.eventsourcing.bankAccount.events.BalanceWithdrawedEvent;
...
public class BankAccountAggregate extends AggregateRoot {
    ...
    public void when(Event event) {
        switch (event.getEventType()) {
            ...
            case BalanceWithdrawedEvent.BALANCE_WITHDRAWED ->
                    handle(SerializerUtils.deserializeFromJsonBytes(event.getData(), BalanceWithdrawedEvent.class));
    ...
    private void handle(final BalanceWithdrawedEvent event) {
        Objects.requireNonNull(event.getAmount());
    ...
    public void withdrawBalance(BigDecimal amount) {
        final var data = BalanceWithdrawedEvent.builder().aggregateId(id).amount(amount).build();
        final byte[] dataBytes = SerializerUtils.serializeToJsonBytes(data);
        final var event = this.createEvent(BalanceWithdrawedEvent.BALANCE_WITHDRAWED, dataBytes, null);
        apply(event);
    }
